...
async function handleOrderApproved(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Grant Plantify Pro: set is_premium = true on the buyer's User record.
  // User.update is idempotent, so duplicate ORDER_APPROVED deliveries are safe.
  // Runs before marking the purchase paid: a throw keeps status "pending" so Wix retries.
  if (purchase.appUserId) {
    await db.entities.User.update(purchase.appUserId, { is_premium: true });
  } else if (buyerEmail) {
    // Anonymous buyer: match by email (User records can't be created here).
    const matches = await db.entities.User.filter({ email: buyerEmail });
    const u = matches?.[0];
    if (u) {
      await db.entities.User.update(u.id, { is_premium: true });
    } else {
      console.warn("payments-webhook: no user matched buyer email; cannot grant", { buyerEmail });
    }
  }
  // ===== END APP-SPECIFIC =====
...
async function handleSubscriptionEnded(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Revoke Plantify Pro: set is_premium = false. (One-time purchases don't trigger this,
  // but subscriptions do — handled here for completeness.)
  if (purchase.appUserId) {
    await db.entities.User.update(purchase.appUserId, { is_premium: false });
  }
  // ===== END APP-SPECIFIC =====
